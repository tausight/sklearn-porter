...
class DecisionTreeRegressor(Regressor):
    ...
    def export(self, class_name, method_name, export_data=False,
        ...
        n = len(classes)
        m = self.n_classes
        classes = [', '.join([str(x) for x in e[0]]) for e in classes]
        classes = ', '.join([temp_arr_scope.format(v) for v in classes])
        classes = temp_arr__.format(type='double', name='classes', values=classes,
                                    n=n, m=m)
